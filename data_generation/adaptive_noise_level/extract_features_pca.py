...
def read_image_from_csv(csv_file, num_extract):
    """
    Read image paths from a CSV file and randomly sample if the number exceeds `num_extract`.

    Args:
        csv_file (str): Path to the CSV file containing at least an 'image' column.
        num_extract (int): Number of rows to extract; -1 means all rows.

    Returns:
        list: images, a list of image paths (strings).
    """
    images = []
    ...
    if num_extract == -1 or num_extract >= len(images):
        sampled = images
    else:
        sampled = random.sample(images, num_extract)
