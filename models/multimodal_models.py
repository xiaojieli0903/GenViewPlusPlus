...
class VisionTextEncoder(nn.Module):
    ...
    def forward(self, img_cat, label, text, *args, **kwargs):
        ...
        images = torch.cat(augs, dim=0) # [n_captions * n_views, 3, H, W]
        feats = self.visual(images)
        feats = torch.split(feats, n_caption, dim=0)    # n_views * [n_captions, D]

        # do separate BN
        h = None
        if hasattr(self, 'image_mlp'):
